...
#IN situ data
situ<-read.csv(paste("Z:/1_OWASIS/2_Statistical_analysis/rf_controls/0_RAAM/0_Data/rm_ts_all_daily/",stn,"_day.csv",sep=""),header=TRUE)
situ<-situ[,1:6]
...
#change print time to
pri<-grep("TPrint",select)+1 #read next line
print<-end.sim
select[pri]<-end.sim
...
new.nam<-paste("Balance",suff,".out",sep="")
file.rename("Balance.out",new.nam)
