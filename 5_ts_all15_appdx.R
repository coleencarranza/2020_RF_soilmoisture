...
ins.plot<-function(x,col,mod.var){
  ...
  axis(side=2, at=seq(mn,mx,by=0.1), las=2,cex.axis=0.7, lwd=0.15)
  axis(side=1, at=seq(mn,mx,by=0.1),cex.axis=0.8, lwd=0.15,)
  abline(0,1, lty =4)
}
if(h1d=="da"){
  suff<-"_DA"
...
pdf(paste("./3_figs/ts",type,suff,"_append5.pdf"), height=8,width=16.5)
st<-as.Date("2016-01-01", format = "%Y-%m-%d")
fi<-as.Date("2020-02-21", format = "%Y-%m-%d")
yr.seq<-seq.Date(st,fi, by= "year")[1:4]


par(oma = c(4,4.5,2.5,0.5))
layout(matrix(c(1:15),ncol=3, byrow =TRUE))
for(i in seq_along(min.max)){
  x<-min.max[[i]]
  ##TIME SERIES
  par(mar=c(0.3,0,0,0.3), mgp = c(1.5,0.65,0), tck = -0.035)
  plot(as.zoo(x$VWC_zone), ylim=c(0,0.46), type = "l",lwd=1.5, xlim=c(st,fi), col="black",xaxt= "n",yaxt="n", bty="n")
  #Hydrus
  ...
  if(i %in% c(1,4,7,10,13)){
    axis(side=2, at=seq(0,0.4,by=0.1), labels = c(0.0,"",0.2,"",0.4), las=2,cex.axis=1.2, lwd=0.25)
  }
  ...
  ##Accuracy text 
  legend('bottomright', ncol = 3L, x.intersp = 0.15,xpd=NA, inset = c(-0.01,0),cex =0.9,y.intersp = 0.85,
  ...
  par(mgp = c(1.3,0.3,0), tck = -0.07)
  subplot(ins.plot(ins,col=co.rf,"RF"),  x=as.Date("2019-04-30", format = "%Y-%m-%d"),y=0.38, size = c(0.4,0.4))
  
  #in situ vs H1D
  ins<-data.frame(x[complete.cases(x$vl.Q50),c("VWC_zone",h1d.run)])
  ins<-ins[complete.cases(ins),]
  subplot(ins.plot(ins,col=co.hyd,"oBOFDA"),  x=as.Date("2019-12-20", format = "%Y-%m-%d"),y=0.38, size = c(0.4,0.4))

}

leg<-as.Date("2015-11-30", format = "%Y-%m-%d")
legend(x=leg, y=-0.1, legend =c("RF", "oBOFDA","in situ"), horiz = TRUE, xpd =NA, text.width = c(150,150,200),
...
mtext(side=2, expression(paste("Root zone soil moisture  (",theta["rz"], ", ",m^3*m^-3,")")), outer=TRUE, line=2.1, cex =1.25)
if(type =="pred"){
  mnt<-"Interpolation"
}else if(type =="fcast"){
  mnt<-"Extrapolation"
