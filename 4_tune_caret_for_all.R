...
#for case weights,extreme values
...
rf_out<-paste0("./1_RF/RF_",type,".ranger_mtry1_25.RData")#_inflec_cw_hpc
load(rf_out)
#mods_tune<-rf_out
...
mod.tol<-  ranger(VWC_zone ~ .-VWC5 -VWC5_lag -VWC5_lag3 -VWC5_lag40  ,
                        data = tr.all[[1]],
...
qrf.mod <- ranger(VWC_zone ~ .-VWC5 -VWC5_lag -VWC5_lag3 -VWC5_lag40  ,
                  data = tr.all[[1]],
                  mtry=best.mod.tol$mtry,
                  num.trees = round(nrow(tr.all[[1]])*0.10,-2),
                  min.node.size = best.mod.tol$min.node.size,
...
save(mods_tune,
    ...
    vl.xts,
    samps,
    file = paste0("./1_RF/RF_",type,".ranger_mtry1_25_inflec.RData"))
