...
load("./data/all_raam/RF_fcast.ranger_mtry1_25.RData")
...
plot(varImp(fcast.mod.tol),sort=TRUE,type= 1,cex= 1, pt.cex =1.3, pch=22, bg = "lightblue",frame.plot = FALSE, lcolor="grey40", main = "",cex.lab = 1.25,lwd=2)
...
#-------------------------------------PLOT!-------------------------------------------------
#pdf(paste("./figs/qrf_fcast_ts_min_max.tol.pdf"), height=4,width=7)
...
#setEPS()
#postscript(paste("./figs/scat_resid_fcast.tol.eps"), height=3,width=10.5)
...
mtext(side =3, "Forecasting", outer=TRUE, line =-1.75,cex = txt.acc,font=3, col ="blue4")#Hydrus-1D
