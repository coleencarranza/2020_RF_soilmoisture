...
for(i in seq_along(train.prop)){
  pred.samps[[i]]<-lapply(dmy.list, function(x) tr.vl(df=x,samp=train.prop[i]))
}

tr<-lapply(pred.samps,function(x)  lapply(x, function(y) y[[1]]))
...
st<-Sys.time()
pred_mods_tune<-lapply(tr.all, tune_rf)
end<-Sys.time()
...
vl<-lapply(pred.samps,function(x) lapply(x, function(y) y[[2]]))[[1]]
vl<-lapply(vl, function(x) x[complete.cases(x),])
vl<-lapply(vl,function(x) {x$rf_pred<-predict(pred.mod.tol,x, type = "response")[[1]];x})


######----------------------------Quantreg with Ranger------------------------
#Use tuned hyper parameters--------
#PRED
#use model using 0.5 training
rf_mod05<-pred_mods_tune[[1]]


## Quantile regression forest - training set
...
##Validation set - xts
vl2<-lapply(pred.samps,function(x) lapply(x, function(y) y[[2]]))[[1]]
vl2<-lapply(vl2, function(x) x[complete.cases(x),])


qrf.pred.vl2 <- lapply(vl2, function(x) predict(qrf.mod,x, type = "quantiles", quantiles = c(0.05, 0.5, 0.95)))
qrf.pred.vl2<-lapply(qrf.pred.vl2, function(x) x$predictions)


#-----------------MErge vl and qrf------------------------------------
pred_vl<-list()
for(i in seq_along(vl)){
  pred_vl[[i]]<-cbind(vl[[i]],data.frame(qrf.pred.vl2[[i]]))
}
...
##To xts
library(xts)
pred_vl<-lapply(pred_vl, function(x) {x$date<-as.Date(x$date,origin ="1970-01-01");x})
pred_vl.xts<-lapply(pred_vl, function(x) xts(x[,-1],x[,1]))

##test plot
par(mfrow =c(4,2), mar = c(0,0,0,0))
lapply(pred_vl.xts, function(x) {plot(as.zoo(x$VWC_zone), type = "b",pch=19,cex=0.7, ylim =c(0.05,0.45))
                            ...
                            points(as.zoo(x$rf_pred), col = "red3", cex=0.31, ylim =c(0.05,0.45))
...
save(pred_mods_tune,
     ...
     pred_vl.xts,
     pred.samps,
     file = "./data/all_raam/RF_pred.ranger_mtry1_25.RData")
